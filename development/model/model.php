...
require_once SITEPATH.'controller/singleton.php';
...
class MyModel extends model {
   ...
   public function FindUsers($val = "") {
    ...
    function uploadImage($image)
    {
    	...
    	$this->db->fields(array("image_id"=>'',"image"=>$image));
    	$this->db->from("advertisement");
    	$test=$this->db->insert();
    	if($test==1)
    	{
	...
	public function SearchContent($matchfield) {
		...
		//print_r($matchfield); die;
		$this->db->Fields(array("property_id" , "price"));
		$this->db->From("seller_table");
		//$this->db->Join("property_image","a.property_id=b.property_id");
...
//		echo $matchfield1;
		...
		if($count==1)
		$this->db->where(array("seller_table.property_id"=>$matchfield1['property_id']));
		else if($count==2)
			$this->db->where(array("seller_table.property_id"=>$matchfield1['property_id'],"seller_table.price"=>$matchfield1['price']));
...
public function viewMessage($val = "") {
		...
		$this->db->Fields ( array ("message_time","message_subject","message_content","assign_name") );
...
public function FinalProperty($value)
	...
	public function SendMessage($value)
	{
		...
		$this->db->Fields(array("assign_name"=>$_SESSION['user_name'],"assigned_by"=>$_SESSION['user_id'],
